...
public class Worker : Unit
{
    ...
    public int Gather(GameMap map)
    {
        ...
        if (tile.ResourceAmount <= 0 || tile.ResourceYield is null) return 0;
        if (CarriedAmount >= CarryCapacity) return 0;
        if (CarriedResource is not null && CarriedResource != tile.ResourceYield) return 0;

        int space = CarryCapacity - CarriedAmount;
        int amount = Math.Min(space, Math.Min(tile.ResourceAmount, 5)); // gather up to 5 per action
    ...
    /// </summary>
    public (ResourceType type, int amount)? Deposit(ResourceInventory inventory)
    {
        if (CarriedAmount == 0 || CarriedResource is null) return null;

        var result = (CarriedResource.Value, CarriedAmount);
